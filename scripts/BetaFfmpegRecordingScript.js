...
const MainCode = async () => {
  ...
  // Obtain audio stream using getUserMedia
  const audioStream = await navigator.mediaDevices.getUserMedia({ audio: true });
  const HandlerAudioTrack = audioStream.getAudioTracks()[0];

  const HandlerCombinedTracks = new MediaStream();
  HandlerCombinedTracks.addTrack(HandlerVideoTrack);
  HandlerCombinedTracks.addTrack(HandlerAudioTrack);

  const RecorderMedia = new MediaRecorder(HandlerCombinedTracks, { mimeType: 'video/webm;codecs=h264' });
  var TemporaryFrames = []
